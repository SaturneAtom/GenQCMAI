...
def main():
    ...
    if pdf is not None:
        ...
        with tab3:
            # Create powerpoint
            st.write("Création PowerPoint")
            st.button("Créer")

            # Generate description
            if three_generation_result is None:
                    query = "Imaginez-vous être un professeur d'université enseignant un cours basé sur le document fourni. Votre objectif est d'aider les étudiants à mieux comprendre le contenu en formulant des texte à trous pertinentes."
                    ...
                    with get_openai_callback() as cost:
                        three_generation_result = chain.run(input_documents=docs, question=query)
                        print(cost) 


            st.write("This is the Owl tab")
            description = three_generation_result
